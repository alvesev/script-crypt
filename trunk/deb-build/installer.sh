...
install --owner=root --group=root --mode=755 --verbose "${DIR_ORIGIN}/rename-camera-files"              "${CHROOT}/usr/bin/rename-camera-files"
...
install --owner=root --group=root --mode=755 --verbose "${DIR_ORIGIN}/reduce-image-dpi"                 "${CHROOT}/usr/bin/reduce-image-dpi"
